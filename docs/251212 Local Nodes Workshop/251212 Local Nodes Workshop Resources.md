# ReFi Local Nodes Workshop - Resource Compilation

**Created:** November 2025  
**Purpose:** Comprehensive resource document for workshop participants  
**Last Updated:** November 2025

---

## Quick Start Links

### Essential First Steps
- [ReFi DAO Garden](https://app.gardens.fund/gardens/42220/0x4f604735c1cf31399c6e711d5962b2b3e0225ad3/0x13e71c56c5b048e9b8b6a9dbb4a4f346b5dab986) - Join & sign Community Covenant
- [Local Node Onboarding Checklist](https://www.notion.so/ReFi-DAO-Local-Node-Onboarding-Checklist-2082e7251f2f80a9be95f674baa25646) - Step-by-step guide
- [Membership Agreement](https://www.notion.so/ReFi-DAO-Local-Node-Membership-Agreement-v1-201bf304370a8038a898e7ba76319fc1) - Review before applying
- [Strategy Call Booking](https://calendly.com/luizfernando_) - Book with Luizfernando

### Funding Opportunities
- [Local Node Kickstarter Pool](https://app.gardens.fund/gardens/42220/0x4f604735c1cf31399c6e711d5962b2b3e0225ad3/0x13e71c56c5b048e9b8b6a9dbb4a4f346b5dab986/48) - Up to 500 USDGLO
- [Network Initiatives Pool](https://app.gardens.fund/gardens/42220/0x4f604735c1cf31399c6e711d5962b2b3e0225ad3/0x13e71c56c5b048e9b8b6a9dbb4a4f346b5dab986) - Up to 3,000 USDGLO

---

## Onboarding Process

### Phase 1: Initial Steps

1. **Join ReFi DAO's Garden**
   - Link: [Gardens Platform](https://app.gardens.fund/gardens/42220/0x4f604735c1cf31399c6e711d5962b2b3e0225ad3/0x13e71c56c5b048e9b8b6a9dbb4a4f346b5dab986)
   - Connect your wallet
   - Click "Join" - requires 1 USDGLO on Celo as registration stake
   - Review and sign the Community Covenant

2. **Review Membership Agreement**
   - Link: [Membership Agreement](https://www.notion.so/ReFi-DAO-Local-Node-Membership-Agreement-v1-201bf304370a8038a898e7ba76319fc1)
   - Understand commitments and benefits
   - Book strategy call: [Calendly](https://calendly.com/luizfernando_)

3. **Submit Proposal on Gardens**
   - Link: [Local Node Kickstarter Pool](https://app.gardens.fund/gardens/42220/0x4f604735c1cf31399c6e711d5962b2b3e0225ad3/0x13e71c56c5b048e9b8b6a9dbb4a4f346b5dab986/48)
   - Can request up to 500 USDGLO
   - Include in proposal:
     - Team bios with relevant skills and experience
     - Project deliverables and cost breakdown
     - Timeframe for completion
     - Relevant links, demos, or resources
   - Proposal template available in resources

### Phase 2: After Approval

1. **Sign Local Node Agreement**
   - Link: [Hats Protocol - Local Node Agreement Signer](https://app.hatsprotocol.xyz/trees/42220/13?hatId=13.2.9)
   - Click "Sign the Agreement"
   - Review and click "Sign Agreement & Claim"
   - Claim Local Node Agreement Signer Hat
   - Await manual assignment of Local Node Lead 2025 Hat

2. **Join Gated Channels**
   - Link: [Guild.xyz - Local Node Onboarding](https://guild.xyz/refidao/onboarding-and-roles)
   - Go through process to mint Local Node Leader Hat
   - Access gated resources and channels (requires Local Node Lead 2025 Hat)

3. **Set Up Branding & Social Channels**
   - Access brand assets (see Brand Assets section below)
   - Essential: Community Telegram Group (add @refidaoglobal as Admin)
   - Recommended: Twitter/X, Farcaster
   - Optional: LinkedIn, WhatsApp, Bluesky, Instagram, Mastodon

4. **Create KarmaGAP Profile**
   - Link: [KarmaGAP](https://gap.karmahq.xyz/)
   - Search for your project or select "Add Project"
   - Post updates on project activities
   - Track metrics and progress over time

5. **Connect with Network**
   - Add @refidaoglobal to your Telegram group as admin
   - Introduce yourself: [Regen Coordination Hub](https://hub.regencoordination.xyz/t/welcome-refi-daoist/220)
   - Join community channels:
     - [Events Calendar](https://lu.ma/user/usr-H16hNq7qfyf0j8T)
     - [Forum](https://hub.regencoordination.xyz/c/refi-dao/21)
     - [Socials](https://linktr.ee/refidao)

---

## Brand Assets & Design Resources

### Official Brand Assets
- **Link:** [ReFi Brand Assets (Google Drive)](https://drive.google.com/drive/folders/1k5zEQT7fBUA-Z1IQxFw60f8wE-H6amF1)
- **Contents:**
  - ReFi DAO logos in various formats (PNG, SVG)
  - Typography guidelines and font files
  - Color palette and brand standards
  - Social media templates

### Local Node Brand Kits
- **Link:** [Local Node Brand Kits (Google Drive)](https://drive.google.com/drive/folders/1EjexyIkorsbV_hVLxBF5JwG63PeShIuv)
- **Contents:**
  - Location-specific logo variations
  - Customizable templates for over 25+ Local Nodes
  - Social media assets and profile images
  - Event materials and promotional content

### Event & Website Graphics
- **Link:** [ReFi DAO Events Graphics (Figma)](https://www.figma.com/design/g9OFvnMoZqa72gsYdVu5Ml/ReFi-DAO-Event-and-Website-Graphics?node-id=845-4335&p=f&t=8IyFGqqHqQlVMMoC-0)
- **Contents:**
  - Event banner templates
  - Website graphics and UI elements
  - Social media story templates
  - Presentation templates

### Brand Guidelines & Best Practices

**Visual Identity Standards:**
- Use official ReFi DAO color palette as base
- Incorporate local cultural elements thoughtfully
- Maintain logo hierarchy and spacing requirements
- Follow typography guidelines for consistency

**Communication Guidelines:**
- Align messaging with ReFi DAO Community Covenant
- Reflect local context while maintaining global mission
- Use consistent tone and language across channels
- Include proper attribution to ReFi DAO network

**Social Media Best Practices:**
- Cross-promote with other Local Nodes
- Share learnings and successes with the network
- Tag @ReFiDAO in relevant posts
- Use network hashtags: #ReFi #RegenerativeFinance #LocalNode

---

## Platform Access & Accounts

### Gardens Platform
- **Purpose:** Decentralized funding and governance
- **Link:** [ReFi DAO Garden](https://app.gardens.fund/gardens/42220/0x4f604735c1cf31399c6e711d5962b2b3e0225ad3/0x13e71c56c5b048e9b8b6a9dbb4a4f346b5dab986)
- **Features:**
  - Join community and sign covenant
  - Submit funding proposals
  - Participate in conviction voting
  - Track proposals and funding pools

### Guild.xyz
- **Purpose:** Gated access to resources and channels
- **Link:** [Local Node Onboarding](https://guild.xyz/refidao/onboarding-and-roles)
- **Features:**
  - Mint Local Node Leader Hat
  - Access gated Telegram channels
  - Access private resource folders
  - Verify membership status

### KarmaGAP
- **Purpose:** Project tracking and impact documentation
- **Link:** [KarmaGAP](https://gap.karmahq.xyz/)
- **Features:**
  - Create project profile
  - Post activity updates
  - Track metrics over time
  - Document achievements and deliverables

### Hats Protocol
- **Purpose:** On-chain role verification
- **Link:** [Local Node Agreement Signer](https://app.hatsprotocol.xyz/trees/42220/13?hatId=13.2.9)
- **Features:**
  - Sign Local Node Agreement on-chain
  - Claim Local Node Agreement Signer Hat
  - Verify official Local Node Leader status

### Regen Coordination Hub
- **Purpose:** Public forum discussions
- **Link:** [ReFi DAO Forum](https://hub.regencoordination.xyz/c/refi-dao/21/none)
- **Features:**
  - Participate in discussions
  - Share updates and learnings
  - Ask questions and get support
  - Connect with other nodes

---

## Communication Channels

### Local Node Leaders Telegram
- **Access:** Via Guild.xyz (requires Local Node Lead 2025 Hat)
- **Purpose:** Private group for coordination and support
- **Usage:** Strategy discussions, resource sharing, updates

### ReFi DAO Open Telegram Group
- **Link:** [Join Telegram Group](https://t.me/+UogSLqbkwIphNjM0)
- **Purpose:** Public community discussions
- **Usage:** General questions, introductions, community updates

### Regen Coordination Hub
- **Link:** [ReFi DAO Forum](https://hub.regencoordination.xyz/c/refi-dao/21/none)
- **Purpose:** Public forum discussions
- **Usage:** Detailed discussions, documentation, Q&A

### Monthly Leadership Calls
- **Purpose:** Strategy and network updates
- **Frequency:** Monthly
- **Access:** Calendar invites sent to Local Node Leaders

### Support Channels
- **Direct Contact:** @luizfernando (Telegram)
- **Questions Channel:** [Telegram Support](https://t.me/+9SqORyW9gIw2Y2E0)
- **Strategy Calls:** [Book with Luizfernando](https://calendly.com/luizfernando_)

---

## Documentation & Guides

### Onboarding Documentation
- **Onboarding Checklist:** Step-by-step checklist for new nodes
- **Membership Agreement:** Terms and commitments for Local Nodes
- **Starting Guide:** Comprehensive guide for starting a Local Node
- **Proposal Template:** Template for Gardens funding proposals

### Internal Resources
- **Member Resources Hub:** Complete guide to resources and support
- **Contribution Points System:** Earn recognition for network contributions
- **Funding Pools Documentation:** Details on funding opportunities

### Toolkits & Guides
- **Local Node Toolkit:** Implementation guides and best practices
- **Brand Guidelines:** Visual identity and communication standards
- **Event Planning:** Resources for organizing local events

---

## Funding & Support

### Funding Pools

**Local Node Kickstarter Pool**
- **Amount:** Up to 500 USDGLO
- **Purpose:** Initial activities for new nodes
- **Use cases:** Events, workshops, community-building initiatives
- **Link:** [Submit Proposal](https://app.gardens.fund/gardens/42220/0x4f604735c1cf31399c6e711d5962b2b3e0225ad3/0x13e71c56c5b048e9b8b6a9dbb4a4f346b5dab986/48)

**Network Initiatives Pool**
- **Amount:** Up to 3,000 USDGLO
- **Purpose:** Larger initiatives and collaborations
- **Use cases:** Network-wide projects, tool development, research
- **Link:** [Submit Proposal](https://app.gardens.fund/gardens/42220/0x4f604735c1cf31399c6e711d5962b2b3e0225ad3/0x13e71c56c5b048e9b8b6a9dbb4a4f346b5dab986)

### Support Programs

**Mentorship Program**
- Connect with experienced Local Node leaders
- Learn from established nodes
- Get guidance on challenges

**Technical Support**
- Access to technical resources
- Guidance on tool implementation
- Integration support

---

## Tracking & Reporting

### KarmaGAP Activity Tracking
- **Link:** [KarmaGAP Project Tracking](https://gap.karmahq.xyz/community/refidao)
- **Purpose:** Document Local Node progress and achievements
- **Usage:** Post updates, track metrics, share deliverables

### Contribution Points System
- **Link:** [Contribution Points Documentation](https://www.notion.so/ReFi-DAO-Reputation-Contribution-Points-Documentation-2082e7251f2f80a9be95f674baa25646?pvs=21)
- **Purpose:** Earn recognition for network contributions
- **Usage:** Track contributions, build reputation, access benefits

### Quarterly Impact Reports
- **Purpose:** Share Local Node achievements with network
- **Frequency:** Quarterly
- **Format:** Standardized reporting template

---

## Additional Resources

### Events & Calendar
- **Link:** [ReFi DAO Events Calendar](https://lu.ma/user/usr-H16hNq7qfyf0j8T)
- **Purpose:** Join upcoming events and calls
- **Usage:** Discover network events, RSVP, add to calendar

### Social Media
- **Link:** [ReFi DAO Linktree](https://linktr.ee/refidao)
- **Platforms:** Twitter, Telegram, Farcaster, LinkedIn
- **Usage:** Follow updates, engage with community, share content

### External Resources
- **Local ReFi Toolkit:** Implementation guides and case studies
- **Regenerative Finance Resources:** Educational materials
- **Web3 Tools:** Technical documentation and guides

---

## Support & Contact

### Primary Support Contact
- **Name:** Luizfernando
- **Telegram:** @luizfernando
- **Email:** luizfernando@refidao.com
- **Strategy Calls:** [Book Appointment](https://calendly.com/luizfernando_)

### Support Channels
- **Telegram Support:** [Questions Channel](https://t.me/+9SqORyW9gIw2Y2E0)
- **Forum:** [Regen Coordination Hub](https://hub.regencoordination.xyz/c/refi-dao/21/none)
- **Local Node Leaders Telegram:** Via Guild.xyz access

### Common Questions
- **Wallet/Hat changes:** Use [form](https://tally.so/r/mZZR0B) or contact @luizfernando
- **Access issues:** Contact through Telegram or forum
- **Proposal questions:** Book strategy call or post in forum

---

## Next Steps After Workshop

1. **Complete onboarding checklist items**
   - Join Gardens and sign covenant
   - Review membership agreement
   - Book strategy call if needed

2. **Submit proposal within 2-4 weeks**
   - Use proposal template
   - Include all required information
   - Request appropriate funding amount

3. **Join monthly calls**
   - Receive calendar invites
   - Participate in network updates
   - Share your progress

4. **Connect with network**
   - Join communication channels
   - Introduce yourself on forum
   - Start building relationships

5. **Set up your Local Node**
   - Access brand assets
   - Set up social channels
   - Create KarmaGAP profile
   - Plan first activities

---

*This document is regularly updated. Bookmark for easy reference!*  
*Last updated: November 2025*

